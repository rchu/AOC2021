...
const puzzle1 = [596598, [
    'on x=-45..1,y=-25..23,z=-14..35',
...
const puzzle2 = [1199121349148621, [
    'on x=-45..1,y=-25..23,z=-14..35',
    'on x=-30..18,y=-39..14,z=-1..48',
    'on x=-40..10,y=-45..5,z=-47..7',
    'on x=-25..19,y=-21..32,z=-10..35',
...
function sizeCube($cube):int {
    return (abs($cube[x][0] - $cube[x][1]) + 1) * (abs($cube[y][0] - $cube[y][1]) + 1) * (abs($cube[z][0] - $cube[z][1]) + 1);
}

function strCube($cube):string {
    return sprintf("[%s %2d..%2d %2d..%2d %2d..%2d %4d]", 
        $cube[on] ? 'ON ' : 'OFF',
        $cube[x][0], $cube[x][1],
        $cube[y][0], $cube[y][1],
        $cube[z][0], $cube[z][1],
...
function readInput($input) {
    return array_map(function($line) {
        ...
        $cube = [
            on => $matches[1] == 'on',
            x    => [intval($matches[2]), intval($matches[3])],
            y    => [intval($matches[4]), intval($matches[5])],
            z    => [intval($matches[6]), intval($matches[7])],
        ];
        foreach ([x,y,z] as $axis) sort($cube[$axis], SORT_NUMERIC);
        return $cube;
    }, array_reverse($input));
}

function countOnCubes($cubes, $depth=0, $bounding_cube = []): int {
    if (!$cubes) return 0;
    $doneCubes = [];
    $onCount = 0;
    $cubeCount = 0;
    foreach ($cubes as $cube) {
        ...
        $overlapCubes = [];
        if ($cube[on]) {
            foreach ($doneCubes as $doneCube) {
                # $done overlaps with $cube
                if (    (($doneCube[x][1] >= $cube[x][0]) and ($doneCube[x][0] <= $cube[x][1]))
                    and (($doneCube[y][1] >= $cube[y][0]) and ($doneCube[y][0] <= $cube[y][1]))
                    and (($doneCube[z][1] >= $cube[z][0]) and ($doneCube[z][0] <= $cube[z][1]))
                )
                #add trimmed $done
                $overlapCubes[] = [
                    on=>true,
                    x => [ max($cube[x][0], $doneCube[x][0]), min($cube[x][1], $doneCube[x][1]) ],
                    y => [ max($cube[y][0], $doneCube[y][0]), min($cube[y][1], $doneCube[y][1]) ],
                    z => [ max($cube[z][0], $doneCube[z][0]), min($cube[z][1], $doneCube[z][1]) ],
                ];
            }
            $onCountPlus = sizeCube($cube);
            $onCountMinus = countOnCubes($overlapCubes, $depth+1, $cube);
            $onCount = $onCount + $onCountPlus - $onCountMinus;
        }
        $doneCubes[] = $cube;
    }
    return $onCount;
...
foreach ([puzzle1, puzzle2] as $input)
{
    $result = countOnCubes(readInput($input[1]));
    print($input[0]." expected\n$result cubes are ON\n");
}